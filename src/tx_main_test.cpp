...
void i2c_scan() {
  ...
  byte foundAddresses[10]; // Store up to 10 found addresses
  int foundCount = 0;
  ...
  for(address = 1; address < 127; address++ ) {
    ...
    if (error == 0) {
      ...
      // Check if this is the expected OLED address
      if (address == i2c_Address) {
        Serial.print("  -> This matches expected SH1106 address (0x");
        if (i2c_Address < 16) 
          Serial.print("0");
  ...
  // If expected device not found, loop forever with error message
  if (!expectedFound) {
    Serial.println("\n*** ERROR: Expected I2C device not found! ***");
    ...
    if (foundCount > 0) {
      Serial.print("Actual addresses found: ");
      ...
      Serial.println("Actual addresses found: NONE");
    }
    
    // Loop forever with 1 second delay
    while (true) {
      Serial.println("\n--- Expected vs Actual ---");
